...
// Cделать логирование при регистрации и логинации в register и login

const app = express();
const port = 3000;

// Более старая версия middleware, её импортировали из body-parser, сейчас используется встроенный express.json()
...
app.put("/certificates/:id", authenticateJWT, (req: Request, res: Response) => {
  ...
  const { statusCode, dateChange, validFrom, validTo } = req.body; // Ожидаем, что в body будет передан новый statusCode
  ...
  try {
    ...
    // Обновление validTo сертификата
